    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-[1.4fr_1fr]">
        <section className="panel p-6">
          <p className="text-[11px] uppercase tracking-[0.2em] text-[var(--gold)]">What should I do</p>
          ...
          <p className="mt-2 text-lg text-[var(--ink-soft)]">
            {output.windowDaysLow}–{output.windowDaysHigh} day window · {output.confidence}% confidence
          </p>
          <p className="mt-4 max-w-xl text-[15px] leading-relaxed">{output.rationale[0]} {output.rationale[1]}</p>
        ...
        <section className="panel p-6 space-y-4">
          <Split />
          <Row label="Commercial runway" value={`${data.runway.toFixed(0)} days`} hint="Private" />
          <Row label="Stockpile" value={data.stockpile.status === "within_requirement" ? "Within requirement" : "Short"} hint="Private" />
          <Row label="Working capital in inventory" value={moneySgd(data.wc.inventoryTiedSgd)} hint="Private" />
          <Row
            label="Vietnam Jasmine"
  ...
  return (
    <div className="grid grid-cols-2 gap-3 text-[12px]">
      <div className="border border-[var(--rule)] p-3">
        ...
        <p className="mt-1 text-[var(--ink-soft)]">Your firm&apos;s data. Never shared.</p>
      </div>
      <div className="border border-[var(--rule)] bg-[#2a2318] p-3 text-[#f4efe4]">
        ...
        <p className="mt-1 opacity-80">Shared market intelligence for participating traders.</p>
      </div>
    </div>
  );
}
